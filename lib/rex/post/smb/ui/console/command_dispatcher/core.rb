...
###
#
# Core smb client commands that provide only the required set of
# commands for having a functional smb client<->hardware  instance.
#
###
class Console::CommandDispatcher::Core
  ...
  def cmd_irb(*args)
    ...
    if expressions.empty?
      ...
      print_status("The 'client' variable holds the smb client\n")
  ...
  #
  # Executes a script in the context of the smb session.
  #
  def cmd_run(*args)
    if args.length.zero?
      cmd_run_help
    ...
    if !words[1] || !words[1].match(/^\//)
      begin
        ...
        [  # We can just use Meterpreters script path
          ::Msf::Sessions::SMB.script_base,
          ::Msf::Sessions::SMB.user_script_base
        ].each do |dir|
  ...
  def cmd_bgrun(*args)
    ...
    # Get the script name
    self.bgjobs[jid] = Rex::ThreadFactory.spawn("SMBBGRun(#{args[0]})-#{jid}", false, jid, args) do |myjid,xargs|
    ...
    print_status("Executed SMB with Job ID #{jid}")
  end
  ...
  @@client_extension_search_paths = [ ::File.join(Rex::Root, "post", "smb", "ui", "console", "command_dispatcher") ]
  ...
  def cmd_shares(*args)
    print_line "fetching the shares..."
    shares = client.list_shares
    print_line "#{JSON.pretty_generate(shares)}"
  end
...
protected
  ...
  def valid_args(args, required_flags=nil)
    if required_flags
      return false if args.include?('-h') || (args & required_flags) != required_flags

      args.each_with_index do |arg, index|
        if required_flags.include?(arg)
          if args[index + 1].nil? || /-[a-zA-Z]/ =~ args[index + 1]
            print_line "Argument #{arg} requires a value"
            print_line
            return false
          end
        end
